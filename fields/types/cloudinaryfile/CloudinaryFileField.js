/*
TODO: CloudinaryFileType actally supports 'remove' and 'reset' actions, but
...
const SUPPORTED_TYPES = ['image/*', 'application/pdf', 'application/postscript','application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' ];
const SUPPORTED_REGEX = new RegExp(/^image\/|application\/msword|application\/vnd.openxmlformats-officedocument.wordprocessingml.document|application\/pdf|application\/postscript/g);

let uploadInc = 1000;
const buildInitialState = (props) => ({
	removeExisting: false,
	uploadFieldPath: `CloudinaryFile-${props.path}-${++uploadInc}`,
...
module.exports = Field.create({
	...
	componentWillReceiveProps (nextProps) {
		// console.log('CloudinaryFileField nextProps:', nextProps);
	},
	...
	getFilename () {
		const { format, public_id } = this.props.value;

		return this.state.userSelectedFile
			? this.state.userSelectedFile.name : `xxx`;
	...
	handleImageChange (e) {
		...
		if (!file.type.match(SUPPORTED_REGEX)) {
			return alert('Unsupported file type. Supported formats are: GIF, PNG, JPG, BMP, ICO, PDF, TIFF, EPS, PSD, SVG, DOC, DOCX');
		}
	...
	renderClearButton () {
		const clearText = this.hasLocal() ? 'Cancel' : 'Remove File';
		...
		return (
			<div key={this.props.path + '_toolbar'} className="image-toolbar">
				<Button onClick={this.triggerFileBrowser}>
					{this.hasImage() ? 'Change' : 'Upload'} File
				</Button>
	...
	renderUI () {
		...
		const imageContainer = (
			<div style={this.hasImage() ? { marginBottom: '1em' } : null}>
				{this.props && this.props.value && this.props.value.public_id && <p>{this.props.value.public_id}</p>}
